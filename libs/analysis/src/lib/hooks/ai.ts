...
export function useOllama() {
  ...
  const analyze = async ({ url = "http://localhost:11434", model = "deepseek-r1:7b", hr = defaultHrDesc, demand = '', resume = '', getResponse = (url: string, bodyString: string): Promise<{ json: () => object }> => {
    ...
    const chat = async () => {
      ...
      const response = await getResponse(url, JSON.stringify(body));
