...
export function useOpenai() {
	...
	const analyze = async ({ url = "http://127.0.0.1:1234", model = "mradermacher/DeepSeek-R1-Distill-Llama-8B-Abliterated-GGUF", hr = defaultHrDesc, demand = '', resume = '', token = '', getResponse = (url: string, bodyString: string): Promise<{ json: () => object }> => {
		const headers = new Headers();
		headers.append(`Authorization`, `Bearer ${token}`);
		headers.append(`Content-Type`, `application/json`);
		return fetch(`${url}/v1/chat/completions`, {
			method: "POST", body: bodyString, headers
		});
	} } = {}): Promise<MatchResult> => {
		const chat = async () => {
