...
export function useExtension() {
  ...
  const analyze = async ({ url = "", model = "", hr = defaultHrDesc, demand = '', resume = '', getResponse = (url: string, body: string | object): Promise<{ json: () => object }> => {
    throw "need pass getResponse method";
  } } = {}): Promise<MatchResult> => {
    const chat = async () => {
      const response = await getResponse(url, createBody({ model, hr, demand, resume }));
      ...
      if (content) {
        try {
          jsonText = (content.match(/```json(?<json>[\s\S]*)```/)?.groups?.json) as string;
          matchResult = JSON.parse(jsonText);
        } catch (e) {
          // fix Qwen3-1.7B-q4f32_1-MLC
          // no ```json ``` tag
          try {
            jsonText = (content.match(/<think>[\s\S]*<\/think>(?<json>[\s\S]*)/)?.groups?.json) as string;
            matchResult = JSON.parse(jsonText);
          } catch (e) {
