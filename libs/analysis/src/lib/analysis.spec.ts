...
describe('analysis', () => {
  ...
  it('should display success', async () => {
    ...
    await expect
      ...
      .toBeVisible();
    await expect
      .poll(() => page.getByText('匹配度').element(), { timeout: 1000 })
      .toBeVisible();
    await expect
      .poll(
        () =>
          page
            .getByTitle(
              '候选人的工作经历在网络设备维护和桌面运维方面有一定的经验，能够满足部分岗位需求，但缺乏思科华为技术支持经验和2年以上IT桌面维护经验。此外，他的性格外向要求未得到体现。',
            )
            .element(),
        { timeout: 1000 },
      )
  ...
  it('should display error', async () => {
    ...
    await expect
      ...
      .toBeVisible();
    await expect
      .poll(() => page.getByText('分析失败').element(), { timeout: 1000 })
      .toBeVisible();
  });
