...
use crate::sync::import_company::CompanyImporter;
use crate::sync::types::SyncResult;

pub struct CompanySync;
...
impl CompanySync {
    pub async fn sync(
        ...
        let dir_path = format!("{}/{}", datetime.format("%Y-%m-%d"), "company");
                ...
                for file in xlsx_files {
                    if let Some(download_url) = &file.download_url {
                        match client.download_file(download_url).await {
                            Ok(data) => {
                                match FileParser::parse_company_file_from_bytes(&data) {
                                    Ok(rows) => {
                                        total_records += rows.len();
                                        match CompanyImporter::import(conn, rows).await {
                                            Ok(result) => {
