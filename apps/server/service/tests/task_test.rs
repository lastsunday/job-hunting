...
use service::task::{
    RepoType, TaskPlanConfigDataDownloadConfig, TaskType, Type, create_plan,
    filter_sort_fetch_date_info, get_file_name_by_task_type, query_date_list,
...
async fn test_task_create_and_gen() {
    ...
    let now: DateTime<Utc> = "2024-01-01T00:00:00Z".parse::<DateTime<Utc>>().unwrap();
    let file_name = get_file_name_by_task_type(TaskType::JobDataDownload);
    // TODO: key not static,need to query other table
    let TaskPlanConfigDataDownloadConfig { url, key, .. } = config;
    ...
    let repo_file_name_and_max_seq_map =
        query_repo_file_date_and_max_seq_map(file_name.as_str(), url, key, now, retention_day)
            .await
            .unwrap();
    let (start_date, end_date, repo_asc_date_list) =
        filter_sort_fetch_date_info(repo_file_name_and_max_seq_map);
    ...
    let lack_date_max_seq_list = calculate_lack_date_max_seq_list(
        repo_asc_date_list,
        db_date_list,
        repo_file_name_and_max_seq_map,
    )
    .await;
    // 4. 根据缺失日期及对应maxSeq，user_name,repo_name,task_type,生成 data download task
    save_data_download_task(task_type, lack_date_max_seq_list, url, user_name, repo_name)
        .await
        .unwrap();
