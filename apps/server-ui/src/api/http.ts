...
const refreshToken = async (refresh_token: string | null): Promise<Token> => {
  const { status, data } = await axios.post(
    `${import.meta.env.VITE_BASE_URL}/api/auth/access_token`,
    undefined,
    {
      params: {
...
export async function putJson<T>(url: string, obj: Object): Promise<T> {
  const { status, data } = await instance.put(url, JSON.stringify(obj));
  if (status == HttpStatusCode.Ok || status == HttpStatusCode.BadRequest) {
    if (data) {
      const code = data.code;
      if (code == 0) {
...
export async function deleteJson<T>(url: string, params?: any): Promise<T> {
  const { status, data } = await instance.delete(url, { params });
  if (status == HttpStatusCode.Ok || status == HttpStatusCode.BadRequest) {
    if (data) {
      const code = data.code;
      if (code == 0) {
