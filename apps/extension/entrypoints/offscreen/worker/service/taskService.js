...
import { genInTextSql, handleAndReturnWhereSql } from "./sqlUtil";
...
export const SERVICE_INSTANCE = new BaseService("task", "id",
  () => {
    return new Task();
  },
  () => {
...
export const _updateTaskStatus = async ({ param = null, connection = null } = {}) => {
  connection ??= await getDb();
  const id = param.id;
  const status = param.status;
  const sql = `UPDATE task SET status = '${status}' ${handleAndReturnWhereSql(genInTextSql(id, 'id'))}`;
