...
export const saveTask = async ({
  ...
  userName,
  repoName,
  getTotalByTaskType = _getTotalByTaskType,
}) => {
  await (
    await getDb()
  ).transaction(async (tx) => {
    const total = await getTotalByTaskType({
      type,
      startDatetime,
      endDatetime,
      connection: tx,
    });
    const pageSize = await _getPageSizeByTaskType({ type });
...
export const _getTotalByTaskType = async ({
  type,
  startDatetime,
  endDatetime,
  connection,
...
export const _getPageSizeByTaskType = async ({ type }) => {
  if (type == TASK_TYPE_JOB_DATA_UPLOAD) {
...
export async function addDataUploadTask({
  type,
  startDatetime,
  endDatetime,
  userName,
  ...
  taskDataUpload.dataPageNum = pageNum;
  taskDataUpload.dataPageSize = pageSize;
  const savedTaskDataUpload = await _taskDataUploadAddOrUpdate({
    param: taskDataUpload,
    connection,
  });
  const task = new Task();
  task.type = type;
...
export async function calculateRepoMaxUploadDate({
  ...
  const filterResult = await queryRepoFileDateList({
    userName,
    repoName,
    taskType: type,
    getPathMap,
...
export async function searchByChunk({
  param = {},
  connection = null,
  searchFunction = null,
  maxChunkCount = null,
  ...
  if (param.pageNum == null && param.pageSize == null && maxChunkCount) {
    ...
    const queryTotal = (
      await searchFunction({ param: queryTotalParam, connection })
    ).total;
    const result = [];
    const stepCount =
      Number.parseInt(queryTotal / maxChunkCount) +
      (queryTotal % maxChunkCount > 0 ? 1 : 0);
    for (let i = 0; i < stepCount; i++) {
      const stepQueryParam = Object.assign(param, {
        pageNum: i + 1,
        pageSize: maxChunkCount,
      });
      const stepItems = (
        await searchFunction({ param: stepQueryParam, connection })
      ).items;
...
export async function getJobData({
  ...
  return searchByChunk({
    param: searchParam,
    connection,
    searchFunction: _searchJob,
    maxChunkCount: JOB_MAX_EXPORT_SIZE,
  });
}
...
export async function getCompanyData({
  ...
  return searchByChunk({
    param: searchParam,
    connection,
    searchFunction: _searchCompany,
    maxChunkCount: COMPANY_MAX_EXPORT_SIZE,
  });
}

export async function getCompanyTagData({
  pageNum = null,
  pageSize = null,
  startDatetime,
  endDatetime,
  ...
  return searchByChunk({
    param: searchParam,
    connection,
    searchFunction: _companyTagExport,
    maxChunkCount: COMPANY_TAG_MAX_EXPORT_SIZE,
  });
}

export async function getJobTagData({
  pageNum = null,
  pageSize = null,
  startDatetime,
  endDatetime,
  ...
  return searchByChunk({
    param: searchParam,
    connection,
    searchFunction: _jobTagExport,
    maxChunkCount: JOB_TAG_MAX_EXPORT_SIZE,
  });
}

export async function getJobPublicData({
  pageNum = null,
  pageSize = null,
  startDatetime = null,
  endDatetime = null,
  ...
  return searchByChunk({
    ...
    connection,
    searchFunction: _searchJob,
    maxChunkCount: JOB_PUBLIC_MAX_EXPORT_SIZE,
  });
}
...
export async function createRepoIfNotExists({
  ...
  try {
    await GithubApi.getRepo(userName, repoName, {
    ...
    if (e == EXCEPTION.NOT_FOUND) {
      await GithubApi.newRepo(repoName, {
        isPrivate,
        getTokenFunction: getToken,
        setTokenFunction: setToken,
      });
...
export async function uploadData({
  ...
  if (dataList.length > 0) {
    infoLog(
      `[Task Data Upload ${dataTypeName}] data length = ${dataList.length}`
    );
    ...
    const seq = pageNum <= 1 ? '' : `_${pageNum - 1}`;
    const filePath = `${dirPath}/${dataTypeName}${seq}.zip`;
    try {
      await GithubApi.createFileContent(
        userName,
        repoName,
        filePath,
        zipData,
        `upload ${dataTypeName} file,local datetime=${dayjs().format()}`,
        { getTokenFunction: getToken, setTokenFunction: setToken }
      );
      infoLog(
        `[Task Data Upload ${dataTypeName}] create file ${filePath} success`
      );
    } catch (e) {
      if (e == EXCEPTION.CREATION_FAILED) {
        infoLog(
          `[Task Data Upload ${dataTypeName}] create file ${filePath} failure`
        );
