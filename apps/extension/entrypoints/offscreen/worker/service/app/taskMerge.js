...
import {
  _batchAddOrUpdateCompanyTag,
  _companyTagExport,
} from '../companyTagService';
...
import {
  _jobPublicBatchAddJobPublicAndUpdateJob,
  SERVICE_INSTANCE as JOB_PUBLIC_SERVICE_INSTANCE,
} from '../jobPublicService';
import { _batchAddOrUpdateJob, _jobGetByIds } from '../jobService';
import { _jobTagBatchAddOrUpdate, _jobTagExport } from '../jobTagService';
import {
  _taskDataMergeAddOrUpdate,
  _taskDataMergeGetById,
} from '../taskDataMergeService';
...
export function setup(handleMap) {
  handleMap.set(TASK_TYPE_JOB_DATA_MERGE, async (dataId) => {
    return mergeDataByDataId(
      dataId,
      TASK_TYPE_JOB_DATA_MERGE,
      DATA_TYPE_NAME_JOB,
      JOB_FILE_HEADER,
      jobExcelDataToObjectArray,
      async (items, taskDataMerge, connection) => {
        //处理数据冲突问题，根据创建时间来判断
        //处理公司名全称问题
        let targetList = await getMergeDataListForJob(
          items,
          'jobId',
          async (ids) => {
  ...
  handleMap.set(TASK_TYPE_COMPANY_DATA_MERGE, async (dataId) => {
    return mergeDataByDataId(
      ...
      async (items, taskDataMerge, connection) => {
        ...
        let targetList = await getMergeDataListForCompany(
          items,
          'companyId',
          async (ids) => {
  ...
  handleMap.set(TASK_TYPE_COMPANY_TAG_DATA_MERGE, async (dataId) => {
    return mergeDataByDataId(
      ...
      async (items, taskDataMerge, connection) => {
        //处理数据冲突问题，根据更新时间合并
        let targetList = await getMergeDataListForTag(
          items,
          'companyName',
          async (companyNames) => {
            let searchParam = new CompanyTagExportBO();
            searchParam.source = taskDataMerge.username;
            searchParam.companyIds = companyNames.map((item) =>
              genIdFromText(item)
            );
        ...
        await _batchAddOrUpdateCompanyTag({
          companyTagBOs: targetList,
  ...
  handleMap.set(TASK_TYPE_JOB_PUBLIC_DATA_MERGE, async (dataId) => {
    return mergeDataByDataId(
      ...
      async (items, taskDataMerge, connection) => {
        ...
        const targetObject = await getMergeDataListForJobPublic(
          items,
          'jobId',
          async (ids) => {
            return await _jobGetByIds({ param: ids, connection });
          ...
          async (ids) => {
            const result = await JOB_PUBLIC_SERVICE_INSTANCE._search(
              { jobIds: ids, sourceType: 0, source: taskDataMerge.username },
              { connection }
            );
            return result.items;
          }
        );
        await _jobPublicBatchAddJobPublicAndUpdateJob(targetObject, {
          connection,
        });
...
export async function handleCompanyCommentDataMerge({ dataId = null } = {}) {
  return mergeDataByDataId(
    ...
    async (items, taskDataMerge, connection) => {
      ...
      const targetList = await getMergeDataListForCompanyComment(
        filterList,
        'id',
        async (ids) => {
          return COMPANY_COMMENT_SERVICE_INSTANCE._getByIds(ids, {
            connection,
          });
        }
      );
      ...
      await COMPANY_COMMENT_SERVICE_INSTANCE._batchAddOrUpdate(targetList, {
        connection,
...
export async function handleMetadataMerge({ dataId = null } = {}) {
  ...
  infoLog(
    `[TASK DATA MERGE] taskDataMerge dataId = ${taskDataMerge.dataId},datetime = ${taskDataMerge.datetime},typeId = ${taskDataMerge.typeId}`
  );
  ...
  const dataSourceMetadata = await DATA_SOURCE_METADATA_SERVICE._getById(
    typeId
  );
  dataSourceMetadata.data = data;
  await DATA_SOURCE_METADATA_SERVICE._addOrUpdate(dataSourceMetadata);
  infoLog(
    `[TASK DATA MERGE] merge file name = ${file.name}, id = ${file.id} success`
  );
  return null;
}

export async function mergeDataByDataId(
  dataId,
  taskType,
  dataTypeName,
  fileHeader,
  excelDataToObjectArrayFunction,
  dataInsertFunction
) {
  ...
  const total = taskDataMerge.dataCount;
  const pageNum = taskDataMerge.dataPageNum;
  const pageSize = taskDataMerge.dataPageSize;
  infoLog(
    `[TASK DATA MERGE] taskDataMerge dataId = ${taskDataMerge.dataId},username = ${taskDataMerge.username},repoName = ${taskDataMerge.reponame},datetime = ${taskDataMerge.datetime},pageNum = ${pageNum},pageSize = ${pageSize},total = ${total}`
  );
  const file = await _fileGetById({ param: taskDataMerge.dataId });
  infoLog(`[TASK DATA MERGE] file id = ${file.id},name = ${file.name}`);
  let base64Content = file.content;
  let excelFileBufferData = await getExcelDataFromZipFile(
    base64Content,
    actualFileName
  );
  let wb = read(excelFileBufferData, { type: 'buffer', cellDates: true });
  let validResultObject = validImportData(
    utils.sheet_to_json(wb.Sheets[wb.SheetNames[0]], { header: 1, UTC: true }),
    fileHeader
  );
  if (!validResultObject.validResult) {
    infoLog(
      `[TASK DATA MERGE] valid file name = ${file.name}, id = ${file.id} failure`
    );
    return `文件校验失败，缺少数据列(${
  ...
  infoLog(
    `[TASK DATA MERGE] valid file name = ${file.name}, id = ${file.id} success`
  );
  ...
  let rowStartIndex = (pageNum - 1) * pageSize;
  let rowEndIndex = Math.min(pageNum * pageSize, total);
  if (rowStartIndex > dataTotal || rowEndIndex > dataTotal) {
    return `文件读取失败，读取范围越界，total = ${dataTotal},startIndex = ${rowStartIndex},endIndex = ${rowEndIndex}`;
  }
  infoLog(
    `[TASK DATA MERGE] dataTotal = ${dataTotal}, rowStartIndex = ${rowStartIndex},rowEndIndex = ${rowEndIndex}`
  );
  await (
    await getDb()
    ...
    let count = await mergeByChunk({
      taskDataMerge,
      items: data.slice(rowStartIndex, rowEndIndex),
      mergeFunction: async ({ items }) => {
        return await dataInsertFunction(
          excelDataToObjectArrayFunction(items, taskDataMerge.datetime, {
            config,
          }),
          taskDataMerge,
          tx
    ...
    infoLog(
      `[TASK DATA MERGE] merge file name = ${file.name}, id = ${file.id} success,data count = ${count}`
    );
  });
}
...
const mergeByChunk = async ({ taskDataMerge, items, mergeFunction } = {}) => {
  ...
  const stepSize = DEFAULT_MAX_MERGE_SIZE;
  const stepTotalRound =
    Number.parseInt(total / stepSize + '') + (total % stepSize > 0 ? 1 : 0);
  infoLog(
    `[TASK DATA MERGE] Task dataId = ${taskDataMerge.id},taskType = ${taskDataMerge.type},dataTotal = ${total},stepSize = ${stepSize},stepTotalRound = ${stepTotalRound}`
  );
  for (let i = 0; i < stepTotalRound; i++) {
    const start = i * stepSize;
    const end = Math.min(total, (i + 1) * stepSize);
    const stepItems = items.slice(start, end);
    infoLog(
      `[TASK DATA MERGE] Task dataId = ${taskDataMerge.id},taskType = ${taskDataMerge.type},stepRound = ${i},start = ${start},end = ${end},stepItems.length = ${stepItems.length}`
    );
    totalResult += await mergeFunction({ items: stepItems });
  }
  return totalResult;
};
