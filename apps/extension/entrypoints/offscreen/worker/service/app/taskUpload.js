...
// Calculate
export async function calculateUploadTask({
  userName,
  repoName,
  taskType,
  targetDay = async () => {
  ...
  const taskDataUploadMaxDateString = await _taskDataUploadGetMaxEndDatetime({
    username: userName,
    reponame: repoName,
    type: taskType,
  });
  const taskDataUploadMaxDate = taskDataUploadMaxDateString
    ? dayjs(taskDataUploadMaxDateString).startOf('day')
    : null;
  const today = (await targetDay()).startOf('day');
  if (today.isSame(taskDataUploadMaxDate)) {
    //如果存在
    infoLog(
      `[TASK DATA UPLOAD CALCULATE] taskDataUploadMaxDate is today = ${taskDataUploadMaxDate}`
    );
    infoLog(`[TASK DATA UPLOAD CALCULATE] skip add data upload record`);
    return false;
  } else {
    infoLog(
      `[TASK DATA UPLOAD CALCULATE] taskDataUploadMaxDate(${taskDataUploadMaxDate}) not equal today(${today})`
    );
    ...
    try {
      ...
      if (repoMaxDate && taskDataUploadMaxDate) {
        dataSyncStartDatetime = dayjs.min(
          taskDataUploadMaxDate,
          dayjs(repoMaxDate)
        );
      } else {
        if (repoMaxDate == null) {
      ...
      infoLog(
        `[TASK DATA UPLOAD CALCULATE] dataSyncStartDatetime = ${dataSyncStartDatetime}`
      );
      try {
        infoLog(
          `[TASK DATA UPLOAD CALCULATE] add data upload task ${userName}/${repoName} startDatetime=${dataSyncStartDatetime} endDatetime=${today} starting`
        );
        infoLog(
          `[TASK DATA UPLOAD CALCULATE] add data upload task ${userName}/${repoName} type=${taskType}`
        );
        await saveTask({
          type: taskType,
          startDatetime: dataSyncStartDatetime,
          endDatetime: today,
          userName,
          repoName,
        });
...
export async function uploadDataByDataId(
  dataId,
  dataTypeName,
  getDataFunction,
  ...
  if (!(await isLogin())) {
    debugLog(
      `[TASK HANDLE]No login info, skip run task dataId = ${dataId}, dataTypeName = ${dataTypeName}`
    );
  ...
  return await uploadData({
    ...
    dataTypeName,
    dataList: (
      await getDataFunction({ pageNum, pageSize, startDatetime, endDatetime })
    ).items,
    jsonObjectToExcelJsonArrayFunction,
    pageNum,
  });
