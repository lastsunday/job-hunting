...
export function renderTimeTag(
  ...
  if (platform && platform == PLATFORM_BOSS) {
    ...
    if (firstPublishTime) {
      ...
      const firstPublishTimeHumanReadable =
  ...
  if (platform == PLATFORM_BOSS) {
    ...
    if (jobDTO.hrActiveTimeDesc) {
      ...
      minDatetime = dayjs.min(
        dayjs(hrActiveDatetime),
        dayjs(jobDTO.createDatetime),
      );
    }
    divElement.style = getRenderTimeStyle(minDatetime);
  } else {
    const minDatetime = dayjs.min(
      dayjs(jobDTO.jobFirstPublishDatetime),
      dayjs(jobDTO.createDatetime),
    );
...
export function finalRender(
  jobDTOList,
  { platform, isFinalRender = true, isRecommendPage },
) {
  for (let i = 0; i < jobDTOList.length; i++) {
    ...
    if (commentWrapperDiv) {
      ...
      if (isFinalRender && i == jobDTOList.length - 1) {
        commentWrapperDiv.appendChild(
          $(`<div class="__status_job_render_finish"></div>`)[0],
        );
      }
...
export function createCompanyCommentButton(keyword, keywordSha256) {
  ...
  const button = $(
    `<div class="__comment_button" style="anchor-name:--${buttonAnchorName};">公司评论</div>`,
  )[0];
  const menu = $(`<div
  ...
  const load = () => {
    renderCompanyComment({
      button,
      badgeWrapper,
      menu,
      companyName: keyword,
      companyIdSha256: keywordSha256,
    });
  };
  load();
  return result;
...
const renderCompanyComment = async ({
  button,
  badgeWrapper,
  menu,
  companyName,
  ...
  try {
    ...
    const searchResult = await queryCompanyComment({
      pageNum: 1,
      pageSize: 0,
      companyIdSha256,
    });
    ...
    if (total > 0) {
      badgeWrapper.appendChild(
        $(
          `<div class="__comment_badge __comment_badge_exists">${total}</div>`,
        )[0],
      );
    } else {
      badgeWrapper.appendChild(
        $(`<div class="__comment_badge __comment_badge_not_found">0</div>`)[0],
      );
    }
    const pageSize = UI_DEFAULT_PAGE_SIZE;
    const totalPage =
      Number.parseInt(total / pageSize + '') + (total % pageSize > 0 ? 1 : 0);
    let pageNum = 1;
    renderCompanyCommentContent({
      pageNum,
      pageSize,
      button,
      menu,
      ...
      if (!loading) {
        ...
        if (scrollTop == menuHeight - menu.clientHeight) {
          if (pageNum < totalPage) {
            pageNum = pageNum + 1;
            await renderCompanyCommentContent({
              pageNum,
              pageSize,
              button,
              menu,
    ...
    button.title = '访问异常';
    badgeWrapper.appendChild(
      $(`<div class="__comment_badge __comment_badge_error">❕</div>`)[0],
    );
    throw e;
  }
};
const queryCompanyComment = async ({
  pageNum,
  pageSize,
  companyIdSha256,
} = {}) => {
...
const renderCompanyCommentContent = async ({
  pageNum,
  pageSize,
  companyIdSha256,
  button,
  ...
  const queryResult = await queryCompanyComment({
    pageNum,
    pageSize,
    companyIdSha256,
  });
...
<div class="__company_comment_row_header"><span>${
        (pageNum - 1) * pageSize + index + 1
      }. </span><span>${item.companyName}</span> 评论来自:<span><${
        item.sourceDataName
      }></span> 更新时间:${convertTimeToHumanReadable(
        item.updateDatetime,
      )}</div>
...
export function genCommentTextButton(
  commentWrapperDiv,
  buttonLabel,
  dialogTitle,
  id,
  { autoLoad = false, platform, isRecommendPage, jobCardItemDom } = {},
) {
    ...
    renderCommentContent(
      {
        ...
        id,
        getDataCallback: async ({ first, after, last, before, id }) => {
          clearAllChildNode(commentBadgWrapper);
          //LOADING
          commentBadgWrapper.appendChild(
            $(
              `<div class="__comment_badge __comment_badge_loading">⌛︎</div>`,
            )[0],
          );
          ...
          try {
            ...
            if (data?.search?.issueCount && data?.search?.issueCount > 0) {
              ...
              for (let i = 0; i < data.search.nodes.length; i++) {
                ...
                summary += `${i + 1}: ${author.login}(${dateToStr(
                  createdAt,
                  'YYYY-MM-DD HH:mm:ss',
                )}) >> ${bodyText}\n`;
              }
              ...
              commentBadgWrapper.appendChild(
                $(
                  `<div class="__comment_badge __comment_badge_exists">${data.search.issueCount}</div>`,
                )[0],
              );
            } else {
              ...
              commentBadgWrapper.appendChild(
                $(
                  `<div class="__comment_badge __comment_badge_not_found">0</div>`,
                )[0],
              );
            }
            return data;
            ...
            commentBadgWrapper.appendChild(
              $(
                `<div class="__comment_badge __comment_badge_error">❕</div>`,
              )[0],
            );
            throw e;
          }
  ...
  const commentBadgWrapper = $(
    `<div class="__comment_badge_wrapper" style="anchor-name:--${buttonAnchorName};"></div>`,
  )[0];
...
async function renderCommentContent(
  { first, after, last, before, id, getDataCallback } = {},
  contentDiv,
) {
  const loadingLabel = $('<div>正加载评论⌛︎</div>')[0];
  const loadingDiv = $(`<div class="__comment_loading"></div>`).append(
    loadingLabel,
  )[0];
  ...
  if (!loginInfo) {
    ...
    const installLogin = $(
      `<div>(如需添加评论，请到后台管理[设置]页面安装GitHubApp)</div>`,
    );
    const loginDiv = $(`<div class="__comment_loading"></div>`)
      .append(login)
      .append(installLogin)[0];
  ...
  try {
    ...
    if (total == 0) {
      ...
      contentDiv.appendChild(
        createCommonPageOperationMenu(
          pageInfo.hasPreviousPage,
          pageInfo.hasNextPage,
          pageInfo.startCursor,
          pageInfo.endCursor,
          total,
          async ({ first, after, last, before } = {}) => {
            renderCommentContent(
              { first, after, last, before, id, getDataCallback },
              contentDiv,
            );
    ...
    contentDiv.appendChild(
      createAddCommentRow(
        ...
        () => {
          renderCommentContent(
            { first: COMMENT_PAGE_SIZE, id, getDataCallback },
            contentDiv,
          );
        },
        id,
    ...
    <div class="__comment_row">
      ...
      <div class="__right">
        <div class="__header">
          ...
          <div class="__username" title="${dayjs(
            lastEditedAt || createdAt,
          ).format(
            'YYYY-MM-DD HH:mm:ss',
          )}">更新于${convertTimeOffsetToHumanReadable(
            lastEditedAt || createdAt,
          )}</div>
...
function createAddCommentRow(
  ...
  const submitComment = $(
    `<div class="__comment_submit_button_wrapper"><div class="__comment_submit_button">提交评论</div></div>`,
  )[0];
...
function createCommonPageOperationMenu(
  hasPreviousPage,
  hasNextPage,
  startCursor,
  endCursor,
  ...
  if (hasPreviousPage) {
    const element = $(
      `<div class="__company_info_quick_search_button __comment_paging_button">上一页</div>`,
    )[0];
  ...
  if (hasNextPage) {
    const element = $(
      `<div class="__company_info_quick_search_button __comment_paging_button">下一页</div>`,
    )[0];
    element.addEventListener('click', async (event) => {
...
export function renderSortJobItem(
  list,
  getListItem,
  { platform, orderStartIndex, isRecommendPage },
) {
  ...
  list.forEach((item, index) => {
    ...
    const styleString =
      'order:' +
      (idAndSortIndexMap.get(item.renderSortCustomId) + orderStartIndex) +
      ';';
...
export function sortJobList(list, { platform }) {
  ...
  if (platform == PLATFORM_BOSS || platform == PLATFORM_LIEPIN) {
    ...
    sortList.sort((o1, o2) => {
      return (
        convertHrActiveTimeDescToOffsetTime(o1.hrActiveTimeDesc) -
        convertHrActiveTimeDescToOffsetTime(o2.hrActiveTimeDesc)
      );
    });
    sortList.sort((o1, o2) => {
      return (
        dayjs(o2.createDatetime).valueOf() - dayjs(o1.createDatetime).valueOf()
      );
    });
  }
  if (platform != PLATFORM_BOSS) {
...
export async function renderFunctionPanel(
  ...
  stopAndCleanAbortFunctionHandler();
  const jobTagDTOArray = await JobApi.jobTagGetAllDTOByJobIds(
    list.map((item) => item.jobId),
  );
  ...
  list.forEach((item, index) => {
    ...
    targetDom
      .querySelectorAll(`.__${platform}_function_panel`)
...
async function asyncRenderOtherJobTag(div, item, jobIdAndDTOMap) {
  const jobTagDTOList = (jobIdAndDTOMap.get(item.jobId) ?? []).filter(
    (item) => item.sourceType == TAG_SOURCE_TYPE_CUSTOM && item.source != null,
  );
  ...
  return $(`<div style="display:flex;align-items: center;background-color:${getRandomColor()};border-radius: 3px;padding: 3px;margin-right: 5px;" 
    ...
    ${
      item.isPublic
        ...
        : '<iconify-icon icon="material-symbols:private-connectivity"></iconify-icon>'
    }(${item.sourceList.length}${item.self ? '*' : ''})${item.tagName}
    </div>`)[0];
}
...
function createMyJobTag(item, jobIdAndDTOMap) {
  ...
  asyncRenderTag(
    ...
    '职位',
    async () => {
      return (jobIdAndDTOMap.get(item.jobId) ?? []).filter(
        (item) =>
          item.sourceType == TAG_SOURCE_TYPE_CUSTOM && item.source == null,
      );
    },
    async (tags) => {
...
function createCommentWrapper(jobDTO) {
  ...
  const browseChart = $(
    `<div class="__browse_wrapper">👁️ 展示 ${browseCount} | 🖱️ 查看 ${browseDetailCount}</div>`,
  )[0];
  browseChart.title = `职位展示: ${browseCount}\n职位查看: ${browseDetailCount}`;
  ...
  const quickSearchHandle = async (forceSyncData) => {
    try {
      ...
      if (!item.isFullCompanyName && getCompanyInfoFunction) {
        ...
        let { companyName, jobDescription } = await getCompanyInfoFunction(
      ...
      try {
        ...
        reputationWrapperDiv.append(
          createCompanyReputation(companyName, () => {
        ...
        commentWrapperDiv.appendChild(
          createCompanyCommentButton(companyName, companyIdSha256),
        );
...
async function asyncRenderCompanyInfo(
  ...
  try {
    ...
    if (
      ...
      company &&
      now.isBefore(
        dayjs(company.updateDatetime).add(COMPANY_DATA_EXPRIE_DAY, 'day'),
      )
    ) {
      //skip
    } else {
      ...
      company = await getCompanyFromCompanyInfo(
        companyInfo,
  ...
  contentDiv.append(
    $(`<div class="__company_info_quick_search_item"></div>`)
      ...
      .append(
        $(
          `<div title='${
            company.regCapitalValue
              ? Number.prototype.toLocaleString.call(
                  Number(company.regCapitalValue),
                )
              : '-'
          }'><div class="__company_info_quick_search_item_label" >注册资本：</div>${convertNumberToHumanReadable(
            company.regCapitalValue,
          )}${
            company.regCapitalCurrency ? company.regCapitalCurrency : ''
          }</div>`,
        ),
      ),
  );
...
export function createSearchCompanyLink(keyword) {
  ...
  const buttonAnchorName = genUniqueId();
  const button = $(
    `<div class="__comment_button" style="anchor-name:--${buttonAnchorName};">其他查询渠道</div>`,
  )[0];
  const menu = $(`<div
...
async function asyncRenderCompanyJobTag(div, companyId) {
  const companyTagDTOList = (
    await CompanyApi.getAllCompanyTagDTOByCompanyId(companyId)
  ).filter(
    (item) =>
      !(item.sourceType == TAG_SOURCE_TYPE_CUSTOM && item.source == null),
  );
...
async function asyncRenderTag(
  div,
  title,
  getAllDTOFunction,
  saveTagFunction,
  getRecentlyTagFunction,
) {
...
export function createCompanyReputation(keyword, companyTagUpdateCallback) {
  ...
  contentDiv.appendChild(
    genCompanyCheckingElement(keyword, companyTagUpdateCallback, {
      title: '跨境小白网',
      sourceTitle: '信息来源:跨境小白网（若比邻网）https://kjxb.org/',
      sourceUrl: `https://kjxb.org/?s=${encodeURIComponent(
        keyword,
      )}&post_type=question`,
      companyTag: TAG_RUOBILIN_BLACK_LIST,
      searchFunction: async (keyword) => {
        return await httpFetchGetTextWithAbort(
          `https://kjxb.org/?s=${encodeURIComponent(
            keyword,
          )}&post_type=question`,
        );
      },
...
function genCompanyCheckingElement(
  keyword,
...
async function asyncRenderCompanyChecking(
  div,
  keyword,
  companyTagUpdateCallback,
  {
...
async function httpFetchJsonWithAbort({
  url,
  body,
  method,
  headers,
  ...
  const result = await httpFetchJson(
    {
      url,
