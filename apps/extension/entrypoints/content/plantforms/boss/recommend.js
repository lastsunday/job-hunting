...
export async function handleBossRecommendData(data, page, pageSize) {
  ...
  if (startIndex != 0) {
    try {
      ...
      await mutationJobContainerLoadingFinish(node, startIndex + data.length, data).then(async (value) => {
      ...
      console.log("refresh page", e);
      await init(data, 0);
      count = data.length;
    }
  } else {
    await init(data, startIndex);
  }
}

async function init(data, startIndex) {
  await mutationContainer(data).then(async (node) => {
