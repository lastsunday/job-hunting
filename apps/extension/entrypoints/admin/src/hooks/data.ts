...
import {
  ...
  JOB_SNAPSHOT_FULL_FETCH_OR_INSERT_MAX_BATCH_SIZE,
} from '@/common/config';
...
import {
  ...
  companyCommentDataToExcelJSONArray,
...
export function useData() {
  ...
  const batchMerge = async (
    list: any[],
    callback: (subList: any[]) => Promise<void>
  ) => {
    const totalBatches = Math.ceil(list.length / DEFAULT_MAX_MERGE_SIZE);
    for (let i = 0; i < totalBatches; i++) {
      const start = i * DEFAULT_MAX_MERGE_SIZE;
      const end = Math.min(start + DEFAULT_MAX_MERGE_SIZE, list.length);
      const rangeList = list.slice(start, end);
      await callback(rangeList);
    }
  };

  const saveJobData = async (data) => {
    const jobList = jobExcelDataToObjectArray(data);
    const targetList = await getMergeDataListForJob(
      jobList,
      'jobId',
      async (ids) => {
    ...
    await batchMerge(targetList, async (subList: any[]) => {
      return await JobApi.batchAddOrUpdateJob(subList);
    });
    return targetList;
  };
  ...
  const saveJobPublicData = async (data) => {
    ...
    await batchMerge(jobPublicList, async (subList: any[]) => {
      const targetObject = await getMergeDataListForJobPublic(
        ...
        async (ids: string[]) => {
          const result = await JobPublicApi.jobPublicSearch({
            jobIds: ids,
            sourceType: 0,
            source: null,
          });
      ...
      return await JobPublicApi.jobPublicBatchAddJobPublicAndUpdateJob(
        targetObject
      );
    });
    return jobPublicList;
  };
  ...
  const saveCompanyData = async (data) => {
    ...
    const targetList = await getMergeDataListForCompany(
      companyBOList,
      'companyId',
      async (ids) => {
    ...
    await batchMerge(targetList, async (subList: any[]) => {
      return await CompanyApi.batchAddOrUpdateCompany(subList);
    });
    return targetList;
  };
  ...
  const saveCompanyTagData = async (data) => {
    ...
    const targetList = await getMergeDataListForTag(
      result,
      'companyName',
      async (companyNames) => {
        const searchParam = new CompanyTagExportBO();
    ...
    await batchMerge(targetList, async (subList: any[]) => {
      return await CompanyApi.batchAddOrUpdateCompanyTag({
        items: subList,
  ...
  const saveCompanyCommentData = async (data) => {
    ...
    await batchMerge(targetList, async (subList: any[]) => {
      return await CompanyCommentApi.companyCommentBatchAddOrUpdate({
        items: subList,
      });
    });
    return targetList;
  ...
  const saveJobTagData = async (data) => {
    ...
    await batchMerge(targetList, async (subList: any[]) => {
      return await JobApi.jobTagBatchAddOrUpdate({
        items: subList,
        overrideUpdateDatetime: true,
      });
    });
  ...
  const saveJobSnapshotData = async (data) => {
    ...
    const targetList = await getMergeDataListForJobSnapshot(
      result,
      'id',
      async (ids) => {
    ...
    const totalBatches = Math.ceil(
      targetList.length / JOB_SNAPSHOT_FULL_FETCH_OR_INSERT_MAX_BATCH_SIZE
    );
    for (let i = 0; i < totalBatches; i++) {
      const start = i * JOB_SNAPSHOT_FULL_FETCH_OR_INSERT_MAX_BATCH_SIZE;
      const end = Math.min(
        start + JOB_SNAPSHOT_FULL_FETCH_OR_INSERT_MAX_BATCH_SIZE,
        targetList.length
      );
      const rangeList = targetList.slice(start, end);
      await JobSnapshotApi.jobSnapshotBatchAddOrUpdate({
        items: rangeList,
        overrideUpdateDatetime: true,
      });
    }
