...
const searchFields = {
  common: [
    ...
    <Col span={8} key="startDate">
      <Form.Item name={`startDate`} label={`成立时间`}>
...
const CompanyView: React.FC = () => {
  ...
  const columns: TableColumnsType<CompanyData> = [
    ...
    },
    {
      ...
      render: (value: CompanyTagDTO[]) => {
        ...
        tagData.map((item, index) => {
          result.push(<CustomTag key={index} item={item}></CustomTag>);
        });
        return (
          <Flex wrap gap={2}>
            {result}
    ...
    {
      title: '注册资本',
      dataIndex: 'regCapitalValue',
      render: (value: number) => (
        <Text>
          {value ? Number.prototype.toLocaleString.call(Number(value)) : '-'}
        </Text>
      ),
      minWidth: 100,
    },
