...
export async function sparseCheckout(
  ...
  if (commitHash) {
    ...
    const existsFilePathList = [];
    paths.forEach(path => {
      if (objects[path]) {
        existsFilePathList.push(path);
      }
    });
    if (existsFilePathList.length > 0) {
      const blobsIdx = await fetchObjects(url, existsFilePathList.map(path => objects[path]?.oid), { getResponseAsyncFunction });
      await Promise.all(existsFilePathList.map(async path => {
