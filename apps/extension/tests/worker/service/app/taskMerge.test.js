...
test('handleMetadataMerge in correct logic', async () => {
  ...
  vi.spyOn(modTaskDataMergeService, '_taskDataMergeGetById').mockImplementation(
    async ({ param }) => {
  ...
  vi.spyOn(modFileService, '_fileGetById').mockImplementation(
    async ({ param }) => {
  ...
  vi.mock(
    '@/entrypoints/offscreen/worker/service/baseService',
    async (importActual) => {
