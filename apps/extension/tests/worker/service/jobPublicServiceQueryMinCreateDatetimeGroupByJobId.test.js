...
test('job public service query min createDatetime group by jobid correct', async () => {
  ...
  expect(dateToStr(parse(result.filter(item => item.jobId == "jobId1")[0].createDatetime))).toBe(dateToStr(parse("2025-05-18")));
  expect(dateToStr(parse(result.filter(item => item.jobId == "jobId2")[0].createDatetime))).toBe(dateToStr(parse("2025-05-17")));
})
