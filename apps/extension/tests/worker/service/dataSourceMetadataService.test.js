...
import {
  METHOD_ADD_OR_UPDATE,
  METHOD_BATCH_ADD_OR_UPDATE,
  METHOD_DELETE_BY_ID,
  METHOD_DELETE_BY_IDS,
  METHOD_GET_BY_ID,
  METHOD_GET_BY_IDS,
  METHOD_SEARCH,
...
test('data source metadata service crud logic correct', async () => {
  ...
  vi.spyOn(modUtil, 'postErrorMessage').mockImplementation(
    async (message, error) => {
      throw error;
    }
  );
  ...
  let itemId = null;
  vi.spyOn(modUtil, 'postSuccessMessage').mockImplementation(
    async (message, data) => {
      const item = data;
  ...
  await Service[Service.getMethodName(METHOD_BATCH_ADD_OR_UPDATE)](
    {},
    { items: [item2] }
  );
  vi.spyOn(modUtil, 'postSuccessMessage').mockImplementation(
